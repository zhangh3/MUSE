...
Result::Result(MUSE *muse) : Pointers(muse)
{
...
void Result::modify_params(int narg, char **arg)
{
...
void Result::set_fields(int narg, char **arg)
{
  deallocate();
  ...
  for (int i = 0; i < nargnew; i++) {
    if (strcmp(arg[i],"step") == 0) {
      addfield("Step",&Result::compute_step,INT);
    } else if (strcmp(arg[i],"elapsed") == 0) {
      addfield("Elapsed",&Result::compute_elapsed, INT);
    } else if (strcmp(arg[i],"elaplong") == 0) {
      addfield("Elapsed",&Result::compute_elaplong, INT);
    } else if (strcmp(arg[i], "dt") == 0) {
        addfield("Dt", &Result::compute_dt, FLOAT);
    } else if (strcmp(arg[i], "time") == 0) {
        addfield("Time", &Result::compute_time, FLOAT);
    } else if (strcmp(arg[i],"cpu") == 0) {
      addfield("CPU",&Result::compute_cpu,FLOAT);
    } else if (strcmp(arg[i],"tpcpu") == 0) {
      addfield("T/CPU",&Result::compute_tpcpu,FLOAT);
    } else if (strcmp(arg[i],"spcpu") == 0) {
      addfield("S/CPU",&Result::compute_spcpu,FLOAT);
    } else if (strcmp(arg[i],"wall") == 0) {
      addfield("WALL",&Result::compute_wall,FLOAT);
    ...
    // copy = at most 8 chars of ID to pass to addfield
      ...
      if (arg[i][0] == 'c') {
        ...
        addfield(arg[i],&Result::compute_compute,FLOAT);
        ...
        addfield(arg[i],&Result::compute_variable,FLOAT);
      }
...
void Result::addfield(const char *key, FnPtr func, int typeflag)
{
...
int Result::add_compute(const char *id, int which)
{
  int icompute;
...
int Result::add_variable(const char *id)
{
  int n = strlen(id) + 1;
...
int Result::evaluate_keyword(char *word, double *answer)
{
...
void Result::compute_dt()
{
    dvalue = muse->system->dt;
}/* ---------------------------------------------------------------------- */

void Result::compute_time()
{
